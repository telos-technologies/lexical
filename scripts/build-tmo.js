...
async function build(
  ...
  const inputOptions = {
    ...
    plugins: [
      ...
      postcss({
        extract: true,
        include: [/\.css$/, /node_modules\/@excalidraw\/excalidraw\/.*\.css$/],
        modules: true,
      }),
      url({
        include: ['**/*.svg', '**/*.png', '**/*.jpg', '**/*.gif'],
        limit: 10000, // inline files < 10kb, copy files > 10kb
      }),
      nodeResolve({
        browser: true,
        exportConditions: ['development', 'production', 'default'],
        extensions,
        preferBuiltins: false,
      }),
      commonjs({
        esmExternals: true,
        include: [/node_modules/, /prettier\/.*\.js$/],
  ...
  const outputOptions = {
    ...
    format,
    // change between es and cjs modules
    freeze: false,
