...
const addSingleLink = function(_start, _end, type) {
  ...
  index.log$1.info("abc78 Got edge...", edge);
  ...
  if (edges.length < (config.maxEdges ?? 500)) {
    index.log$1.info("abc78 pushing edge...");
    edges.push(edge);
  } else {
    throw new Error(
...
const addLink = function(_start, _end, type) {
  index.log$1.info("addLink (abc78)", _start, _end, type);
  let i, j;
...
const updateLink = function(positions, style) {
  positions.forEach(function(pos) {
    ...
    if (pos === "default") {
      ...
      if (index.utils.isSubstringInArray("fill", style) === -1) {
...
const setLink = function(ids, linkStr, target) {
  ids.split(",").forEach(function(id) {
    if (vertices[id] !== void 0) {
      vertices[id].link = index.utils.formatUrl(linkStr, config);
...
const addSubGraph = function(_id, list, _title) {
  ...
  index.log$1.info("Adding", subGraph.id, subGraph.nodes, subGraph.dir);
...
const flowDb = {
  defaultConfig: () => index.defaultConfig.flowchart,
