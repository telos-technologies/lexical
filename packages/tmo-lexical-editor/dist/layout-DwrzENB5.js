...
var graph = require('./graph-C1bSurKb.js');
...
function toFinite(value) {
  ...
  value = TmoLexicalEditor.toNumber(value);
...
function flatten(array) {
  ...
  return length ? graph.baseFlatten(array) : [];
}
...
function baseExtremum(array, iteratee, comparator) {
  ...
  while (++index < length) {
    ...
    if (current != null && (computed === undefined
          ? (current === current && !TmoLexicalEditor.isSymbol(current))
  ...
  while (nested != null && ++index$1 < length) {
    ...
    if (index$1 != lastIndex) {
      ...
      if (newValue === undefined) {
        newValue = TmoLexicalEditor.isObject(objValue)
          ? objValue
...
function compareAscending(value, other) {
  if (value !== other) {
    var valIsDefined = value !== undefined,
        ...
        valIsSymbol = TmoLexicalEditor.isSymbol(value);

    var othIsDefined = other !== undefined,
        othIsNull = other === null,
        othIsReflexive = other === other,
        othIsSymbol = TmoLexicalEditor.isSymbol(other);
...
var sortBy = index.baseRest(function(collection, iteratees) {
  ...
  return baseOrderBy(collection, graph.baseFlatten(iteratees), []);
});
...
function findType2Conflicts(g, layering) {
  ...
  function scan(south, southPos, southEnd, prevNorthBorder, nextNorthBorder) {
    var v;
    graph.forEach(range(southPos, southEnd), function (i) {
      v = south[i];
      if (g.node(v).dummy) {
...
function buildLayoutGraph(inputGraph) {
  ...
  g.setGraph(
    index.merge({}, graphDefaults, selectNumberAttrs(graph$1, graphNumAttrs), pick(graph$1, graphAttrs))
  );
  ...
  graph.forEach(inputGraph.edges(), function (e) {
    ...
    g.setEdge(
      e,
      index.merge({}, edgeDefaults, selectNumberAttrs(edge, edgeNumAttrs), pick(edge, edgeAttrs))
    );
  });
